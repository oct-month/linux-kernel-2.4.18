...
#define MAX_NOTES 5			/* arbitrary, must be at least 5 */
...
static int elf_core_dump(long signr, struct pt_regs * regs, struct file * file)
{
	...
	int numnote = 0;
	struct memelfnote notes[MAX_NOTES];
	struct elf_prstatus prstatus;	/* NT_PRSTATUS */
	elf_fpregset_t fpu;		/* NT_PRFPREG */
	struct elf_prpsinfo psinfo;	/* NT_PRPSINFO */
#if defined(__i386__)	
	extern int dump_extended_fpu( struct pt_regs *regs, elf_fpxregset_t *);
	elf_fpxregset_t sse_regs;
#endif
	...
	notes[numnote].name = "CORE";
	notes[numnote].type = NT_PRPSINFO;
	notes[numnote].datasz = sizeof(psinfo);
	notes[numnote].data = &psinfo;
	numnote ++;
	...
	notes[numnote].name = "CORE";
	notes[numnote].type = NT_TASKSTRUCT;
	notes[numnote].datasz = sizeof(*current);
	notes[numnote].data = current;
	numnote ++;

	/* Try to dump the FPU. */
	if(prstatus.pr_fpvalid = dump_fpu (regs, &fpu))
	{ 	
		notes[numnote].name = "CORE";
		notes[numnote].type = NT_PRFPREG;
		notes[numnote].datasz = sizeof(fpu);
		notes[numnote].data = &fpu;
		numnote ++;
	}

#if defined(__i386__)	
	if(dump_extended_fpu(regs, &sse_regs))
	{
		notes[numnote].name = "LINUX"; // gdb is looking for this value.
		notes[numnote].type = NT_PRXFPREG;
		notes[numnote].datasz = sizeof(sse_regs);
		notes[numnote].data = &sse_regs;
		numnote ++;
	}
#endif
