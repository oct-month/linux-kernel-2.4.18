...
#define return_EFAULT do { Dprintk("-EFAULT at %d:%s.\n", __LINE__, __FILE__); return -EFAULT; } while (0)
	...
	if (__copy_to_user(&u_info->id, &req, sizeof(req)))
		return_EFAULT;
...
static int register_mimetype(user_req_t *u_info)
{
	...
	ret = copy_from_user(&addr, &u_info->object_addr, sizeof(addr));
	if (ret)
		GOTO_ERR_no_unlock;
...
asmlinkage int __sys_tux (unsigned int action, user_req_t *u_info)
{
	...
	switch (action) {
		case TUX_ACTION_CONTINUE_REQ:
			ret = continue_request((int)u_info);
			goto out;
...
eventloop:
		...
		case TUX_ACTION_SEND_BUFFER:
		{
			...
			ret = copy_from_user(&addr,
					&u_info->object_addr, sizeof(addr));
			if (ret)
				GOTO_ERR_unlock;
		...
		case TUX_ACTION_WATCH_PROXY_SOCKET:
		{
			...
			ret = copy_from_user(&fd, &u_info->object_addr,
					sizeof(fd));
			if (ret)
				GOTO_ERR_unlock;
