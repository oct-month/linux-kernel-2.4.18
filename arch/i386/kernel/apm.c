...
battery_hook_fp* apm_battery_hook;
DECLARE_MUTEX(apm_battery_hook_sem);

EXPORT_SYMBOL_GPL(apm_battery_hook);
EXPORT_SYMBOL_GPL(apm_battery_hook_sem);
...
static int apm_get_info(char *buf, char **start, off_t fpos, int length)
{
	...
	/* Some machines report no info via APM but it's still available;
	   provide a hook for such drivers to fill stuff in */
	down(&apm_battery_hook_sem);
	if (apm_battery_hook)
		apm_battery_hook(&battery_status, &battery_flag, &ac_line_status,
		    &percentage, &time_units);
	up(&apm_battery_hook_sem);
