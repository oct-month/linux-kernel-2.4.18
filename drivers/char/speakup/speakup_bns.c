/*
 * speakup_bns.c for linux kernels 2.2.x and speakup
 * 
...
/* These routines are written to control the Braille 'n Speak serial
   speech synthesizer in speech box mode.  They are not ment to be
   thought of as a device driver in that they do not register
   themselves as a chr device and there is no file_operations
   structure.  They are strictly to provide an interface to the bns
   from the speakup screen review package.  */
...
#if (LINUX_VERSION_CODE >= 0x20300)  /* v 2.3.50+ */
...
int check, tmout = SPK_XMITR_TIMEOUT;
  ...
  if ((bns_alive) && (timeouts >= NUM_DISABLE_TIMEOUTS)) {
    bns_alive = 0; 
    timeouts = 0;
    return 0; 
  }
...
static int __init serprobe(int index)
{
	...
	spk_serial_out(0x06);
	mdelay(10);
	if (spk_serial_in() == 0x06) return 0;*/

	synth_release_region(ser->port,8);
	bns_alive = synth_port_tts = 0;
        return -1;
}
...
int i;
  ...
  printk("Probing for Braille 'N Speak.\n");
  if (synth_port_tts)
...
static int synth_alive(void)
{
  if (bns_alive)
    return 1; /* already on */
  else if ((!bns_alive) && (synth_port_tts)) {
    if (wait_for_xmitr() > 0) { /* restart */
      bns_alive = 1;
      synth_write(synth->reinit, strlen(synth->reinit));
      return 2;  /* reenabled */
    } else printk("Braille 'N Speak: can't restart synth\n");
  }
  return 0;
}

static const char init_string[] = "\x05Z\x05\x38P\x05\x43 Braille 'n Speak found\r";
static const char reinit_string[] = "\x05Z\x05\x38P\x05\x43 Braille 'N Speak initialized\r";

static struct spk_variable vars[] =
{{"flush", "\x18", "_", (BUILDER|HARD_DIRECT|NO_USER), "*"},
 {"pitch", "8", "\x05_P", (NUMERIC|HARD_DIRECT|USE_RANGE), "0,16"},
 {"caps_start", "\x05\x31\x32P", "_", 0, "*"},
 {"caps_stop", "\x05\x38P", "_", 0, "*"},
 {"rate", "8", "\x05_E", (NUMERIC|HARD_DIRECT|USE_RANGE), "1,16"},
 {"tone", "8", "\x05_T", (NUMERIC|HARD_DIRECT|USE_RANGE), "0,16"},
 {"volume", "8", "\x05_V", (NUMERIC|HARD_DIRECT|USE_RANGE), "0,16"},
 END_VARS};

static char *config[] =
{"\x18", "\x05\x38P", "\x05\x31\x32P", "\x05\x38P", "\x05\x38\x45",
 "\x05\x38T", "\x05\x38V"};

struct spk_synth synth_bns = {"bns", "Version-0.10", "bns",
