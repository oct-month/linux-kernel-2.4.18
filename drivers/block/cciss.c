...
MODULE_DESCRIPTION("Driver for Compaq SA5xxx Controllers version 2.4.30");
MODULE_SUPPORTED_DEVICE("Compaq SA5i SA5i+ SA532 SA5300 SA5312"); 
MODULE_LICENSE("GPL");
...
static int register_new_disk(int cltr, int opened_vol);
		...
		int length, int *eof, void *data)
{
	...
	for(i=0; i<=h->highest_lun; i++) {
		...
		size = sprintf(buffer+len,
				"cciss/c%dd%d: blksz=%d nr_blocks=%d\n",
...
//					c->commands_outstanding);
	...
	case CCISS_REGNEWD:
		return(register_new_disk(ctlr, dsk));
...
static int register_new_disk(int ctlr, int opened_vol)
{
	struct gendisk *gdev = &(hba[ctlr]->gendisk);
	ctlr_info_t  *h = hba[ctlr];
	int start, max_p, i;
	...
	for(i=0; i<  num_luns; i++) {
		...
		if( lunID_found == 1)
			continue;
		else {	/* It is the new lun we have been looking for */
#ifdef CCISS_DEBUG
