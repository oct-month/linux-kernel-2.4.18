...
void scsi_old_times_out(Scsi_Cmnd * SCpnt)
{
	...
	spin_lock_irqsave(SCpnt->host->lock, flags);
	...
	spin_unlock_irqrestore(SCpnt->host->lock, flags);
 ...
 *  host->lock already held, so we need do nothing here about grabbing
 *  any locks.
 */
static void scsi_request_sense(Scsi_Cmnd * SCpnt)
{
	...
	SCpnt->sc_data_direction = SCSI_DATA_READ;
         ...
         */
        spin_unlock_irq(SCpnt->host->lock);
	scsi_dispatch_cmd(SCpnt);
        spin_lock_irq(SCpnt->host->lock);
}
