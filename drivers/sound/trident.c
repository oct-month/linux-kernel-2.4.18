...
#define DRIVER_VERSION "0.14.10c"
...
static int acquirecodecaccess(struct trident_card *card)
{
	...
	if(!block)
	{
		printk(KERN_NOTICE "accesscodecsemaphore: try unlock\n");
		block = 1;
		goto unlock;
	}
...
static u16 ali_ac97_get(struct trident_card *card, int secondary, u8 reg)
{
	...
	while(1) {
		...
		if(ncount <=0)
			break;
		if(ncount--==1) {
			printk(KERN_NOTICE "ali_ac97_read :try clear busy flag\n");
...
static void ali_ac97_set(struct trident_card *card, int secondary, u8 reg, u16 val)
{
	...
	while(1) {
		...
		if(ncount <= 0)
			break;
		if(ncount-- == 1) {
			printk(KERN_NOTICE "ali_ac97_set :try clear busy flag!!\n");
