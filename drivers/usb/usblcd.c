	  ...
	  unsigned long arg)
{
	...
	case IOCTL_GET_HARD_VERSION:
	  ...
	  sprintf(buf,"%1d%1d.%1d%1d",(i & 0xF000)>>12,(i & 0xF00)>>8,(i & 0xF0)>>4,(i & 0xF));
          if(copy_to_user((void *)arg,buf,strlen(buf))!=0) return -EFAULT;
	  break;
...
{	
	...
	if (dev->descriptor.idVendor != 0x1212) {
		return NULL;
	}
	...
	info("USBLCD Version %1d%1d.%1d%1d found at address %d",(i & 0xF000)>>12,(i & 0xF00)>>8,(i & 0xF0)>>4,(i & 0xF), dev->devnum);
...
int usb_lcd_init(void)
{
	...
	info("USBLCD Driver Version 1.00 (C) Adams IT Services http://www.usblcd.de");
